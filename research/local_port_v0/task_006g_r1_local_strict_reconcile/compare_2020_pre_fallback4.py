"""Compare real BEFORE vs AFTER close fix.

CORRECT comparison:
- BEFORE: r3_full_2020_202605_research/trades_pre_fallback4.csv (before Fallback 4 + close fix)
- AFTER:  r3_2020_only_close_fix/trades.csv (after Fallback 4 + close fix, standalone 2020 run)

The previous compare_2020_close_fix.py was wrong because r3_full_2020_202605_research/trades.csv
was already regenerated AFTER the close fix. The pre_fallback4 backup is the true before-fix state.
"""
import pandas as pd

# 真正的修复前（Fallback 4 + close 修复前）
...
old_equity = pd.read_csv(r'd:\Work Space\他山之石\微盘股\research\local_port_v0\task_005_optimize\runs\r3_full_2020_202605_research\equity_pre_fallback4.csv')

# 修复后（独立 2020 回测，含 Fallback 4 + close 修复）
...
# equity uses 'date' column
old_equity['date'] = pd.to_datetime(old_equity['date'])
old_equity = old_equity[old_equity['date'].dt.year == 2020].copy()
new_equity['date'] = pd.to_datetime(new_equity['date'])

print('=== 2020 年回测结果对比（真正修复前 vs 修复后）===')
print(f'{"指标":<25} {"修复前(pre_fallback4)":<25} {"修复后(2020独立)":<25} {"差异":<15}')
print('-' * 90)
...
price_diff = price_cmp[abs(price_cmp['price_old'] - price_cmp['price_new']) > 0.005]
