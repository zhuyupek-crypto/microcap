"""
TASK-005 P2-2 优化版本：行业分散约束
=====================================

基于 P3_code_cleanup.py 修改：
    单一申万一级行业权重上限 20%（g.industry_concentration_cap = 0.20）

实现方式：
    1. 在 select_smallest_market_cap 选股时，构建 code→l1_code 映射
    2. aggregate_target_values 中按行业聚合权重
    3. 超过 20% 的行业，按比例缩减该行业内所有股票的目标权重

数据来源：直接读 parquet 文件
    D:/Work Space/HData/data/processed/metadata/industry_member.parquet
"""
import os
import pandas as pd
from jqdata import *
...
# 模块级缓存：行业映射（避免每次调用都读 parquet）
_INDUSTRY_MAP_CACHE = None
_INDUSTRY_PARQUET_PATH = r"D:/Work Space/HData/data/processed/metadata/industry_member.parquet"


def _load_industry_map():
    """加载 code → l1_code 映射，模块级缓存。"""
    global _INDUSTRY_MAP_CACHE
    if _INDUSTRY_MAP_CACHE is not None:
        return _INDUSTRY_MAP_CACHE
    try:
        df = pd.read_parquet(_INDUSTRY_PARQUET_PATH)
        # 取每只股票最新的行业记录（out_date 为空的为当前有效）
        if 'out_date' in df.columns:
            df = df[df['out_date'].isna()]
        _INDUSTRY_MAP_CACHE = dict(zip(df['code'], df['l1_code']))
        return _INDUSTRY_MAP_CACHE
    except Exception as e:
        log.warn("industry map load failed: %s" % e)
        return {}
...
def get_stock_industry(stock):
    ...
    # industry_member 中的 code 格式可能是 '000001' 或 '000001.XSHE'
    if code in industry_map:
        return industry_map[code]
    if stock in industry_map:
        return industry_map[stock]
    return None
...
def initialize(context):
    ...
    # P2-2: 行业分散约束
    g.industry_concentration_cap = 0.20  # 单一行业权重上限 20%
...
def aggregate_target_values(context):
    ...
    # P2-2: 行业分散约束
    if g.industry_concentration_cap > 0 and target_values:
        cap_value = g.industry_concentration_cap * total_value
        # 按行业聚合
        industry_totals = {}
        stock_industry = {}
        for stock, value in target_values.items():
            ind = get_stock_industry(stock)
            stock_industry[stock] = ind
            if ind is not None:
                industry_totals[ind] = industry_totals.get(ind, 0.0) + value

        # 缩减超限行业
        for ind, ind_total in industry_totals.items():
            if ind_total > cap_value:
                scale = cap_value / ind_total
                for stock in target_values:
                    if stock_industry.get(stock) == ind:
                        target_values[stock] *= scale
                log.info("[INDUSTRY_CAP] %s total=%.2f cap=%.2f scale=%.4f"
                         % (ind, ind_total, cap_value, scale))

    # 单股集中度上限
    if g.stock_concentration_cap > 0:
        cap_value = g.stock_concentration_cap * total_value
