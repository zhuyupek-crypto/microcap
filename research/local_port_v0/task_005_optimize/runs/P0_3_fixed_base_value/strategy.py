"""
TASK-005 P0-3 优化版本：修复 aggregate_target_values 浮盈反馈问题
================================================================

基于母版 微盘股-母版-20260627.py 修改（独立叠加，不含 P0-1/P0-2）：

问题：
    原代码 phase_value = context.portfolio.total_value / 5
    组合浮盈时新批次 stock_value 变大、浮亏时变小，构成"组合层面追涨杀跌"。

修复：
    每个批次在被选中时记录当时的 base_value = total_value / 5，
    该批次持有期内 stock_value 不变。新批次反映组合新规模。
    aggregate_target_values 时按批次独立累加。

这样：
- 旧批次维持原仓位规模，避免已盈利批次的被动加仓
- 新批次按当前组合规模分配，避免亏损批次的被动减仓
"""
from jqdata import *
...
def initialize(context):
    ...
    # P0-3: 每个批次的 base_value（在该批次被选中时锁定）
...
def trade(context):
    ...
    if due_offsets:
        for offset in due_offsets:
            ...
            # P0-3: 锁定该批次的 base_value（基于当前组合规模 / 批次数）
...
def aggregate_target_values(context):
    """P0-3: 按批次独立计算 stock_value，使用每批次锁定的 base_value。"""
    target_values = {}
