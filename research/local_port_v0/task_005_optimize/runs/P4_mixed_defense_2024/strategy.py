"""
TASK-005 混合防御模式：日历防御 + 信号防御取并集
=================================================

设计思路：
    - 日历防御（1月/4月）作为底线：保留母版的硬编码月份防御
    - 信号防御（20日回撤<=-5%）作为补充：覆盖非日历驱动的回撤
    - 进入防御：日历 OR 信号 任一触发
    - 退出防御：日历 AND 信号 都不触发

目标：
    - 2024年：日历防御(4月)躲过崩盘 + 信号防御补充其他回撤
    - 2025年：日历防御(1月) + 信号防御精准触发，避免信号防御单独使用时的频繁切换
...
def initialize(context):
    ...
    g.min_list_days = 375
    # 混合防御：日历 + 信号
    g.defensive_months = [1, 4]          # 日历防御月份（保留）
    ...
    g.signal_enter_drawdown = -0.05
    g.signal_exit_drawdown = -0.02
    g.value_history = []
    ...
    g._atv_cache = None
    # 防御原因追踪
    g.defense_reason = None  # 'calendar' / 'signal' / 'both'
...
def trade(context):
    ...
    g.value_history.append(current_total)
    if len(g.value_history) > g.signal_lookback:
        g.value_history = g.value_history[-g.signal_lookback:]

    # 混合防御判断
    current_month = context.current_dt.month
    calendar_defensive = current_month in g.defensive_months

    signal_defensive = False
    if len(g.value_history) >= g.signal_lookback:
        peak_20d = max(g.value_history)
        drawdown = (current_total - peak_20d) / peak_20d if peak_20d > 0 else 0
        if g.in_defensive_mode:
            signal_defensive = drawdown < g.signal_exit_drawdown
        else:
            signal_defensive = drawdown <= g.signal_enter_drawdown

    should_be_defensive = calendar_defensive or signal_defensive

    # 追踪防御原因
    if should_be_defensive:
        if calendar_defensive and signal_defensive:
            g.defense_reason = 'both'
        elif calendar_defensive:
            g.defense_reason = 'calendar'
        else:
            g.defense_reason = 'signal'
    else:
        g.defense_reason = None

    # 进入防御
    if not g.in_defensive_mode and should_be_defensive:
        switch_to_defensive(context)
        return

    # 退出防御（日历 AND 信号 都不触发）
    if g.in_defensive_mode and not should_be_defensive:
        for etf in g.defensive_etfs:
            order_target(etf, 0)
        g.in_defensive_mode = False
        log.info("[MIXED_DEFENSE] exit defensive mode")
...
def switch_to_defensive(context):
    ...
    log.info("[MIXED_DEFENSE] enter defensive mode, reason=%s" % g.defense_reason)
...
def report_plan(context):
    ...
    if len(g.value_history) >= g.signal_lookback:
        ...
        current_month = context.current_dt.month
        log.info(
            "days=%s defensive=%s reason=%s drawdown_20d=%.2f%% cal_month=%s signal=%s trade_enabled=%s due_offsets=%s"
            % (g.days, g.in_defensive_mode, g.defense_reason, drawdown * 100,
               current_month, current_month not in g.defensive_months and drawdown <= g.signal_enter_drawdown,
               g.trade_enabled, g.last_due_offsets)
        )
    else:
        log.info(
            "days=%s defensive=%s reason=%s (warming up %d/%d) trade_enabled=%s due_offsets=%s"
            % (g.days, g.in_defensive_mode, g.defense_reason, len(g.value_history), g.signal_lookback,
               g.trade_enabled, g.last_due_offsets)
        )
    for offset in g.phase_offsets:
        log.info("phase offset=%s targets=%s base_value=%.2f"
