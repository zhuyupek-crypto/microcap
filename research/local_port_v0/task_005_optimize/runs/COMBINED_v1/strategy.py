"""
TASK-005 综合优化版本：P0-1 + P0-3 + P1b 叠加
==============================================

叠加三项独立验证有效的优化：
1. P0-1: PriceRelatedSlippage(0.002) + order_volume_ratio=0.1（现实成本）
2. P0-3: 每批次锁定 base_value，避免浮盈反馈（理论更稳健）
3. P1b: 加强财务过滤（三选一）+ 流动性过滤（300万）+ 流通市值（5000万）+ 集中度上限 6%

不包含：
- P0-2 止损（2025 牛市几乎不触发，留待多年回测验证）
- P1 信号驱动防御（数据源待确认，单独验证）
"""
from jqdata import *
...
def aggregate_target_values(context):
    """P0-3 + P1b: 按批次 base_value 计算 + 单股集中度上限。"""
    target_values = {}
