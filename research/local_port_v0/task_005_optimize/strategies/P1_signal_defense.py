"""
TASK-005 P1 信号驱动防御版本（基于 COMBINED_v1）
================================================

设计思路：
    原策略用日历驱动防御（1月、4月），但微盘股回撤并非严格日历驱动
    （如 2024-04 中下旬跨月、2025-04 关税冲击跨月）。

信号设计（不依赖外部指数数据，用组合自身回撤）：
    - 维护 20 个交易日 total_value 历史
    - 进入防御：组合 20 日回撤 <= -5%
    - 退出防御：组合 20 日回撤恢复到 -2% 以内
    - 前 20 天数据不足时不启用信号防御（正常交易）

与日历防御的关系：完全替代（不再用 defensive_months）。
"""
from jqdata import *
...
def initialize(context):
    ...
    # P1: 信号驱动防御参数
    g.signal_lookback = 20          # 回看天数
    g.signal_enter_drawdown = -0.05  # 20 日回撤 -5% 进入防御
    g.signal_exit_drawdown = -0.02   # 20 日回撤恢复到 -2% 退出防御
    g.value_history = []             # total_value 历史序列
...
def trade(context):
    # P1: 更新 value_history（每日 trade 时记录）
    ...
    calendar_defensive = current_month in g.defensive_months  # P1: defensive_months=[] 始终 False
    ...
    if len(g.value_history) >= g.signal_lookback:
        ...
        if g.in_defensive_mode:
            ...
            # 注意：signal_defensive=True 表示"仍需防御"
        else:
            # 未防御：回撤 <= -5% 进入防御
    ...
    if g.in_defensive_mode and not should_be_defensive:
        for etf in g.defensive_etfs:
            order_target(etf, 0)
        g.in_defensive_mode = False
        log.info("[SIGNAL_DEFENSE] exit defensive mode")
...
def switch_to_defensive(context):
    ...
    log.info("[SIGNAL_DEFENSE] enter defensive mode")
...
def report_plan(context):
    ...
    if len(g.value_history) >= g.signal_lookback:
        ...
        drawdown = (current_total - peak_20d) / peak_20d if peak_20d > 0 else 0
        log.info(
            "days=%s defensive=%s drawdown_20d=%.2f%% trade_enabled=%s due_offsets=%s yesterday_HL=%s"
            % (g.days, g.in_defensive_mode, drawdown * 100, g.trade_enabled, g.last_due_offsets, ",".join(g.yesterday_HL_list))
        )
    else:
        log.info(
            "days=%s defensive=%s (warming up %d/%d) trade_enabled=%s due_offsets=%s yesterday_HL=%s"
            % (g.days, g.in_defensive_mode, len(g.value_history), g.signal_lookback, g.trade_enabled, g.last_due_offsets, ",".join(g.yesterday_HL_list))
        )
    for offset in g.phase_offsets:
        log.info("phase offset=%s targets=%s base_value=%.2f"
