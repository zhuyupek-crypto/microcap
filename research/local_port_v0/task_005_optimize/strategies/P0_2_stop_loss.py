"""
TASK-005 P0-2 优化版本：加入个股与组合止损
==========================================

基于母版 微盘股-母版-20260627.py 修改（独立叠加，不含 P0-1 滑点改动）：
1. 个股止损：
   - 单股浮亏 <= -15% → 减半仓
   - 单股浮亏 <= -25% → 清仓
2. 组合止损：
   - 组合总价值从历史峰值回撤 <= -20% → 当日所有持仓减半 + 跳过本轮调仓

止损订单在 rebalance 之前执行，避免与正常调仓冲突。
"""
from jqdata import *
...
def initialize(context):
    ...
    # P0-2: 止损参数
    g.stock_stop_loss_half = -0.15   # 单股 -15% 减半
    g.stock_stop_loss_clear = -0.25  # 单股 -25% 清仓
    g.portfolio_stop_drawdown = -0.20  # 组合 -20% 降仓
    g.peak_total_value = context.portfolio.total_value  # 历史峰值
    g.in_portfolio_stop = False  # 是否处于组合止损状态
...
def trade(context):
    ...
    # P0-2: 止损检查（rebalance 之前）
    portfolio_stop = apply_stop_loss(context)
    ...
    if due_offsets:
        ...
        if portfolio_stop:
            # 组合止损触发：跳过本轮调仓买入，仅保留卖出已生效
            log.info("[PORTFOLIO_STOP] skip rebalance buy this round")
        else:
...
def apply_stop_loss(context):
    """执行个股止损 + 组合止损，返回是否触发了组合止损。"""
    ...
    # 更新历史峰值
    if current_total > g.peak_total_value:
        g.peak_total_value = current_total

    peak = g.peak_total_value
    portfolio_drawdown = (current_total - peak) / peak if peak > 0 else 0
    portfolio_stop = portfolio_drawdown <= g.portfolio_stop_drawdown

    # 个股止损（先于组合止损，便于在组合止损日也清理极端个股）
    for stock in list(context.portfolio.positions.keys()):
        if is_defensive_asset(stock):
            continue
        pos = context.portfolio.positions[stock]
        if pos.avg_cost <= 0 or pos.total_amount <= 0:
            continue
        if not can_sell_today(stock, current_data):
            continue
        pnl_ratio = pos.price / pos.avg_cost - 1
        if pnl_ratio <= g.stock_stop_loss_clear:
            log.info("[STOPLOSS-CLEAR] %s pnl=%.2f%% cost=%.2f price=%.2f"
                     % (stock, pnl_ratio * 100, pos.avg_cost, pos.price))
            order_target_value(stock, 0)
        elif pnl_ratio <= g.stock_stop_loss_half:
            target_half = pos.value * 0.5
            log.info("[STOPLOSS-HALF] %s pnl=%.2f%% target=%.2f"
                     % (stock, pnl_ratio * 100, target_half))
            order_target_value(stock, target_half)

    # 组合止损：所有非防御持仓减半
    if portfolio_stop and not g.in_portfolio_stop:
        log.info("[PORTFOLIO_STOP] drawdown=%.2f%% peak=%.2f current=%.2f → reduce all by half"
                 % (portfolio_drawdown * 100, peak, current_total))
        for stock in list(context.portfolio.positions.keys()):
            if is_defensive_asset(stock):
                continue
            if not can_sell_today(stock, current_data):
            ...
            target_half = pos.value * 0.5
            order_target_value(stock, target_half)
        g.in_portfolio_stop = True
    elif not portfolio_stop and g.in_portfolio_stop:
        # 回撤修复，退出组合止损状态
        log.info("[PORTFOLIO_STOP] drawdown recovered to %.2f%%, exit stop mode"
                 % (portfolio_drawdown * 100))
        g.in_portfolio_stop = False

    return portfolio_stop
