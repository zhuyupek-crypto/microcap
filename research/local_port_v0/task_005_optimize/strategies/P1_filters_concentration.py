"""
TASK-005 P1 优化版本：加强财务过滤 + 流动性过滤 + 单股集中度上限
==================================================================

基于母版 微盘股-母版-20260627.py 修改（独立叠加，不含 P0 系列）：

1. 加强财务过滤（替代原"intentionally weak"过滤）：
   - 资产负债率 < 70%（原 95%）
   - ROE > 0（原 -50）
   - OCF > 0（原 -1e8）
   三选二通过（避免单一指标异常拖累）
2. 加入流动性过滤：
   - 20 个交易日平均成交额 > 1000 万元
   - 自由流通市值 > 1 亿元（用 circulating_market_cap）
3. 单股集中度上限：
   - 同一股票累计目标权重 > 4% 时截断为 4%
   - 截断后剩余资金按比例分配给未触顶的股票

注意：circulating_market_cap 单位为"亿元"，需要 ×1e8 转元。
"""
from jqdata import *
...
def initialize(context):
    ...
    # P1: 过滤参数
    g.liquidity_avg_amount_20d = 1e7   # 20 日均成交额下限：1000 万元
    g.circulating_mcap_min = 1e8       # 流通市值下限：1 亿元
    g.stock_concentration_cap = 0.04   # 单股累计权重上限：4%
...
def select_smallest_market_cap(context):
    ...
    # P1: 流通市值下限过滤（circulating_market_cap 单位为亿元）
...
def apply_liquidity_filter(codes, data_date):
    """P1: 20 日均成交额过滤。codes 应已按市值升序排列。"""
    if not codes:
        return []
    try:
        # 取过去 20 个交易日的成交额 = volume * avg
        df = get_price(
            codes,
            end_date=data_date,
            count=20,
...
def build_risk_filter_map(stocks, data_date):
    ...
    for _, row in df.iterrows():
        ...
        cond_roe = roe > 0
        cond_ocf = ocf > 0
        passed = sum([cond_debt, cond_roe, cond_ocf]) >= 2
        result[stock] = passed
    return result
...
def aggregate_target_values(context):
    ...
    if g.stock_concentration_cap > 0:
        ...
        capped = {s: min(v, cap_value) for s, v in target_values.items()}
        return capped
