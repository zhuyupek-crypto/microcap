"""
TASK-005 P2-1 优化版本：延长批次周期（15→30 天）
=================================================

基于 P3_code_cleanup.py 修改：
    g.phase_cycle = 30（原 15）

设计考量：
    - 原策略每批持有 15 个交易日就强制轮换，与微盘股 alpha 释放周期相悖
    - A 股小市值 alpha 通常 60-180 个交易日才充分释放
    - 延长到 30 天可降低换手率，给 alpha 更多释放时间
    - phase_offsets 仍为 [0, 3, 6, 9, 12]，但 modulo 30 后错位批次间隔变为 3 天×原逻辑
    - 注意：phase_offsets 间隔 3 在 cycle=30 下意味着 5 批次错位仍均匀分布
"""
from jqdata import *
